...
{
    FastNoise _fastnoise; //reference to the fastnoise library
    public Vector3Int _gridSize; // create a 3d grid
    public float _cellSize;
    public Vector3[,,] _flowfieldDirection; // store the calculated flowfield
    public float _increment; // specify the resolution of the noise
    public Vector3 _offset, _offsetSpeed; // control noise to change over time
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < _amountofParticles; i++)
        {
            ...
            while (attempt < 100)
            {
                Vector3 randompos = new Vector3(Random.Range(this.transform.position.x, this.transform.position.x + _gridSize.x * _cellSize),
    ...
    void CalculateFlowFieldDirections()
    {
        ...
        for (int x = 0; x < _gridSize.x; x++)
        {
            ...
            for (int y = 0; y < _gridSize.y; y++)
            {
                ...
                {
                    // getSimplex = [-1,1] -> plus one, noise=[0,2]
